...
with open(os.path.join(THIS_DIR, 'scrabble-words.txt'), 'r') as f:
    lines = f.readlines()
...
d = {}
with open(os.path.join(THIS_DIR, '1gram.txt'), 'r') as f:
    for line in f:
...
with open(os.path.join(THIS_DIR, '2gram.txt'), 'r') as f:
    for line in f:
        ...
        if first_word in d:
            d[first_word].append(second_word)
        else:
            d[first_word] = [second_word]
two_grams_vim_dict = str(d).replace(' ', '')

vim_script = '''" Automatically generated using tools/generate_vimscript.py.
" Do not edit this file! Edit tools/generate_vimscript.py instead.

if exists('g:text_omnicomplete#enable_plugin') && g:text_omnicomplete#enable_plugin == 0
    finish
endif
